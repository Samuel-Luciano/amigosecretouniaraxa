...
module.exports = {
    async create (request, response){      
        try {
            ...
            return response.json({
                "mensagem" : "Grupo criado com exito!",
                "status" : 200,
                "grupo" :grupo            
            }); 
        }       
    ...
    async delete (request, response){
        ...
        return response.json({
            "mensagem" : "Grupo excluido com exito!",
            "status" : 200
        }); 
    },
    async getById (request, response){
        ...
        if (Grupo === null) {
            response.status(404);           
            return  response.json({
                "mensagem" : "Grupo não localizado!",
                "status" : 404,
                "Grupo" :Grupo            
            });         
        }
        return response.json({
            "mensagem" : "Grupo encontrado com exito!",
            "status" : 200,
            "Grupo" :Grupo            
        }); 
    },
    async addParticipante (request, response){      
        try {        
            ...
            return response.json({
                "mensagem" : "Participante adicionado com exito!",
                "status" : 200           
            }); 
        }       
        catch (err) {
    ...
    async delParticipante (request, response){
        ...
        return response.json({
            "mensagem" : "Participante excluido com exito!",
            "status" : 200
        }); 
    },
}